        ...
        "get": {
            ...
            "parameters": [
                {"name": "source", "in": "query", "required": False,
                 "schema": {"type": "string", "enum": ["audit", "events", "game", "stdout", "stderr", "flask", "update"], "default": "game"},
                 "description": "audit=관리 행위 감사로그(DB), events=접속/퇴장 이벤트, game=엔진 로그(stdout 캡처), stderr=NSSM 표준에러, flask=관리자 서버 로그, update=서버 바이너리 업데이트(steamcmd) 로그"},
